...
test('rotates the default bounded scan while newer empty cascades arrive', async () => {
  ...
  try {
    ...
    await Promise.all(initialEmptyCascadeIds.map((id, index) => writeCascade(
      conversationDir,
      id,
      new Date(Date.parse('2026-06-24T02:00:00.000Z') + (index * 1000)).toISOString()
    )))
    const calls: string[] = []
    const options = {
      ...
      requestGeneratorMetadata: async (input: { cascadeId: string }) => {
        ...
        return input.cascadeId === usageCascadeId
          ? generatorMetadataResponse()
          : { generatorMetadata: [] }
      }
    }

    expect(await collectAntigravityGuiUsage({
      ...options,
      collectedAt: '2026-06-24T03:00:00.000Z'
    })).toEqual([])
    expect(calls).toHaveLength(12)
    await Promise.all(newerEmptyCascadeIds.map((id, index) => writeCascade(
      conversationDir,
      id,
      new Date(Date.parse('2026-06-24T04:00:00.000Z') + (index * 1000)).toISOString()
    )))
    expect(await collectAntigravityGuiUsage({
      ...options,
