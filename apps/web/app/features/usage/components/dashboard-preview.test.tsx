...
describe('DashboardPreview', () => {
  test('renders no-cache-read trend and source split metrics', async () => {
    const html = await renderToString(
      <DashboardPreview
        ...
        summary={{
          ...
          deviceCount: 2,
          sourceSplit: [
            { source: 'claude-code', totalTokens: 800, totalTokensWithoutCacheRead: 600 },
            { source: 'codex', totalTokens: 400, totalTokensWithoutCacheRead: 300 }
          ],
          dailyTrend: [
            { usageDate: '2026-04-27', totalTokens: 120, totalTokensWithoutCacheRead: 100, costUsd: 0.12 },
            { usageDate: '2026-04-28', totalTokens: 340, totalTokensWithoutCacheRead: 240, costUsd: 0.34 }
          ]
        }}
      />
    )
    ...
    expect(html).toContain('600 不含缓存读 / 800 total')
    expect(html).toContain('300 不含缓存读 / 400 total')
  })
})
