...
When the server returns an install claim, setup writes `~/.tokenboard/device-link.json` with mode `0600`. Treat this file as sensitive local recovery state. Never print its `installClaim`; status output only reports whether the file exists.
